...
The first excited energy level is obtained by flipping a single spin --- negating its value. % TODO: Caveat for 1d in Section 9.4 below...
...
\begin{center}\includegraphics[width=0.69\textwidth]{figs/unit09_consistency_H.pdf}\end{center} % WARNING: ADJUSTED SIZE BY HAND TO FIT BOTH ON PAGE
...
\begin{center}\includegraphics[width=0.69\textwidth]{figs/unit09_consistency_H-beta.pdf}\end{center} % WARNING: ADJUSTED SIZE BY HAND TO FIT BOTH ON PAGE
...
In addition to revealing that the mean-field approximation fails in one dimension, this result also contradicts our general consideration of the phases of the $d$-dimensional Ising model in \secref{sec:Ising_phases}.
What went wrong there?

The problematic argument in \secref{sec:Ising_phases} was our claim that \textit{all} the degenerate micro-states of the first excited energy level correspond to flipping a single spin to oppose the full alignment of the ground state.
...
Therefore, for $d = 1$ only, our argument that the first excited energy level of the Ising model corresponds to the unique magnetization $|m| = 1 - \frac{2}{N} \to 1$ was incorrect.
As we have now seen from our exact solution, there is actually no ordered phase in one dimension. % TODO: More generic result beyond just Ising model...
