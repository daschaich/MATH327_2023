...
\begin{mdframed}
  \ \\[100 pt]
\end{mdframed}
What is the outcome space $A$ if we toss a coin four times and each time measure whether it lands heads up ($H$) or tails up ($T$)? % Note four tosses is now defined as a single experiment...
\begin{mdframed}
  \ \\[100 pt]
\end{mdframed}
...
To illustrate this, let's again consider the roulette wheel introduced in \secref{sec:prob}.
...
\begin{mdframed}
  \ \\[100 pt]
\end{mdframed}
Since the central limit theorem gives us an approximate \textit{distribution}, in order to compare its prediction against the exact $p_i$ computed above, we need to extract probabilities by integrating over appropriate intervals as discussed in \secref{sec:probdist}.
...
\begin{mdframed}
  \ \\[100 pt]
\end{mdframed}
You can check your results against those plotted below, which come from \href{https://github.com/daschaich/MATH327_2023/blob/master/lecture_notes/unit01_prob.py}{this Python code} that is available for you to peruse, run for different $N$ or modify.
...
(We reserve the variables $\mu$ and $\si^2$ for the mean and variance (respectively) of the single-step process, which will appear when we apply the central limit theorem in \secref{sec:RW_CLT}.)
