...
\renewcommand{\moddate}{Last modified 13 Feb.~2023}
...
We begin this unit by formalizing the concept of \textit{statistical ensembles}, introduced by \href{https://en.wikipedia.org/wiki/Josiah_Willard_Gibbs}{J.\ Willard Gibbs} in the early 1900s.
...
Another important conserved quantity is the total energy $E$ \textit{inside} an isolated (or `closed') system,
\begin{equation*}
  E(\om_1) = E(\om_2).
\end{equation*}
...
That is, if a spin is parallel to the field, it contributes energy $-H$ to the total energy $E$ of the system. % TODO: Note magnetic field strength and energy have same dimensions in this system of units...
...
Instead, the equilibrium system continues probabilistically adopting all possible micro-states as it evolves in time. % TODO: Could reiterate that all accessible micro-states have same energy and particle number...
% ------------------------------------------------------------------
...
\begin{mdframed}
  \ \\[90 pt] % TODO: More space here might be convenient...
\end{mdframed}
% ------------------------------------------------------------------
...
This can be contrasted with \textbf{intensive} quantities, which are \href{https://goldbook.iupac.org/terms/view/I03074}{defined} to be independent of the extent of the system, and hence the same (on average) for subsystems as for the combined system. % External magnetic field strength $H$ is control parameter rather than system property...
...
\begin{itemize} % Adapted from a tutorial exercise since sufficiently important to include in the main material...
...
In so-called \textit{natural} systems, larger energies make more micro-states accessible, producing $\pderiv{M}{E} > 0$ and a positive temperature. % TODO: Remove `so-called'...
...
From \eq{eq:spin_temp} for the temperature of micro-canonical spin systems, we can see that `natural' positive temperatures correspond to negative energies, and therefore increase as the energy increases by becoming less negative (with a smaller magnitude).
...
\begin{center} % WARNING: ADJUSTED SIZE BY HAND TO FILL REMAINDER OF PAGE
...
\begin{align*}
  ...
  S & \in \left\{A, B\right\},
\end{align*}
for simplicity considering the case where the change in energy is relatively small. % TODO: Can generalize by building up big changes from repeated smaller changes...
