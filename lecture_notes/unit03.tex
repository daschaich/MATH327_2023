...
\renewcommand{\moddate}{Last modified 26 Feb.~2023}
...
\end{mdframed}
% This is just the result we would expect from extensivity.
...
\begin{mdframed}
  $\displaystyle 0 = \pderiv{\Sbar}{p_k} = $ \\[100 pt] % WARNING: ADJUSTED SIZE BY HAND TO FILL REMAINDER OF PAGE
\end{mdframed}
...
Applying our second constraint, \eq{eq:canon_Etot}, relates \be to $E_{\text{tot}}$:
\begin{equation} % TODO: Could use different labels for the two sums...
  \label{eq:canon_aveE}
...
What's truly remarkable about Eqs.~\ref{eq:occ_prob}, \ref{eq:part_func} and \ref{eq:beta} is that they make no reference to the $R$ replicas or any extensive quantity such as $E_{\text{tot}}$ --- all information about the thermal reservoir has vanished.
...
\begin{mdframed} % WARNING: ADJUSTED SIZE BY HAND TO FIT ON PAGE
...
For example, in \secref{sec:temp} you also computed the energy $E_1 = -(N - 2)H$ of the first excited state, which is realized by $N$ distinct micro-states (with $n_- = 1$).
