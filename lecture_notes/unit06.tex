...
\renewcommand{\moddate}{Last modified 19 Mar.~2023}
...
\begin{mdframed}
  $\displaystyle 0 = \pderiv{\Sbar}{p_k} = $ \\[140 pt] % WARNING: ADJUSTED SIZE BY HAND TO FILL REMAINDER OF PAGE
\end{mdframed}
...
\begin{mdframed}
  $\displaystyle S(\Etot, \Ntot) = -R \sum_{i = 1}^M p_i \log p_i = $ \\[90 pt] % WARNING: ADJUSTED SIZE BY HAND TO FIT IN PAGE
\end{mdframed}
...
In this notation, \eq{eq:first_law} reads $dE = T \, dS - P \, dV = Q + W$, and relates any changes in the internal energy of a canonical system to changes in its entropy (heat) or volume (work).
